...
export const frameworkDefaults: Record<Framework, FrameworkDefaults> = {
  ...
  angular: {
    redFlags: [
      ...
      'HTTP concerns (auth headers, error handling) duplicated in services instead of HTTP interceptors',
      'Route access control scattered across components instead of route guards',
    ],
    businessLogicIn: ['services'],
    businessLogicForbidden: ['components', 'pipes', 'directives', 'guards', 'interceptors'],
    layerDescriptions: {
      services: SERVICES_DESC,
      components: COMPONENT_DESC,
      ...
      guards: { should: 'Route access control (CanActivate, CanDeactivate, CanLoad)', shouldNot: 'Business logic, data fetching' },
      interceptors: { should: 'Cross-cutting HTTP concerns (auth headers, logging, retries)', shouldNot: 'Business logic, UI concerns' },
      modules: { should: 'Feature composition — declarations, imports, providers', shouldNot: 'Business logic, UI rendering' },
    },
    ...
    ocpExample: 'Use dependency injection, HTTP interceptors, and route guards instead of modifying existing code.',
  },
  vue: {
    redFlags: [
      'Business logic in components — move to composables or services',
      'Direct fetch/axios calls in components — use services or a data fetching composable',
      'Mutating props — use emits or a store',
      'Reactive state not wrapped in ref/reactive/computed',
      'Inline watch logic that should be a composable',
    ],
    businessLogicIn: ['services', 'composables'],
    businessLogicForbidden: ['components', 'pages', 'layouts'],
    layerDescriptions: {
      services: SERVICES_DESC,
      composables: { should: 'Reusable reactive state and logic (ref, computed, watch)', shouldNot: 'Direct DOM manipulation, component rendering' },
      components: COMPONENT_DESC,
      pages: { should: 'Route-level composition of components and data fetching orchestration', shouldNot: 'Business logic, direct API calls' },
      layouts: { should: 'Shared page shells and slots', shouldNot: 'Business logic, route-specific content' },
      directives: { should: 'DOM behavior (v-* directives)', shouldNot: 'Business logic, API calls' },
      plugins: { should: 'App-level installation of global functionality (i18n, router, stores)', shouldNot: 'Business logic, component-specific concerns' },
    },
    ignorePaths: [...BASE_IGNORE, '.env', '.env.*'],
    srpExample: 'A component that renders UI should not also fetch data — extract into a composable or service.',
    dipExample: 'Components should depend on composables or injected services, not imported singletons.',
    ocpExample: 'Use composables, plugins, and slots instead of modifying existing components.',
  },
  nuxt: {
    redFlags: [
      'Secrets exposed in client-side code — use runtimeConfig.server or server routes',
      'Direct database calls in components — use server routes or useFetch',
      'Heavy computation in server components without caching',
      'useFetch inside loops — batch or move to a server route',
      'Mixing server-only and client-only code without explicit boundaries',
    ],
    businessLogicIn: ['services', 'server', 'composables'],
    businessLogicForbidden: ['components', 'pages', 'layouts'],
    layerDescriptions: {
      services: SERVICES_DESC,
      server: { should: 'Server-only code — API routes, server middleware, data access', shouldNot: 'Client-side UI logic, browser APIs' },
      composables: { should: 'Reusable reactive state and logic', shouldNot: 'Server-only operations, direct DB access' },
      components: COMPONENT_DESC,
      pages: { should: 'Route-level composition, data fetching via useFetch/useAsyncData', shouldNot: 'Business logic, direct DB calls' },
      layouts: { should: 'Shared page shells', shouldNot: 'Business logic, route-specific content' },
      plugins: { should: 'App-level plugin installation (client/server/both)', shouldNot: 'Business logic, component concerns' },
    },
    ignorePaths: [...BASE_IGNORE, '.nuxt', '.output', '.env', '.env.*'],
    srpExample: 'A page should not both fetch data AND format it — delegate formatting to a composable or utility.',
    dipExample: 'Pages should depend on composables (useFetch, useAsyncData), not direct $fetch calls to specific URLs.',
    ocpExample: 'Use Nuxt hooks, middleware, and modules instead of modifying existing Nuxt internals.',
  },
  nest: {
    redFlags: [
